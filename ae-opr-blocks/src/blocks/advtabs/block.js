...
import './editor.scss';
    ...
    const { InspectorControls, RichText, PanelColorSettings, InnerBlocks } = wp.blockEditor;
    const { Dashicon, Tooltip, PanelBody, RangeControl, SelectControl, Button } = wp.components;
    ...
    const TABS_STYLES = [
        {name: 'horz', label: __('Horizontal', 'aeopr'), icon: tabHorizontalIcon},
        {name: 'vert', label: __('Vertical', 'aeopr'), icon: tabVerticalIcon},
    ];
    ...
    class AdvTabsWrapper extends Component {
        ...
        componentWillMount() {
            ...
            const currentBlockConfig = aeoprDefaultConfig['aeopr-adv-tabs'];
        ...
        componentDidMount() {
            ...
            if (!this.props.attributes.pid) {
                this.props.setAttributes( {
                    pid: `aeopr-tabs-${this.props.clientId}`,
                } );
            }
        ...
        addTab() {
            ...
            const tabItemBlock = createBlock('aeopr/tab');
        ...
        render() {
            ...
            const blockClass = [
                `aeopr-tabs-wrapper`,
                `aeopr-tab-${tabsStyleD}-desktop`,
                `aeopr-tab-${tabsStyleT}-tablet`,
                `aeopr-tab-${tabsStyleM}-mobile`,
            ].filter( Boolean ).join( ' ' );
            ...
            return (
                isPreview ?
                    <img alt={__('Advanced Tabs', 'aeopr')} width='100%' src={previewImageData}/>
                    :
                <Fragment>
                    <InspectorControls>
                        <PanelBody title={ __( 'Tabs Style', 'aeopr' ) }>
                            <div className="aeopr-columns-responsive-items">
                                {['desktop', 'tablet', 'mobile'].map( (device, index) => {
                                    const itemClasses = [
                                        "aeopr-columns-responsive-item",
                            ...
                            <div className="aeopr-tabs-styles">
                                {TABS_STYLES.map((style, index) => (
                                    <Tooltip key={index} text={style.label}>
                                        <Button className="aeopr-tabs-style"
                        ...
                        <PanelBody title={ __( 'Tabs Settings', 'aeopr' ) }>
                            <SelectControl
                                label={ __( 'Initial Open Tab', 'aeopr' ) }
                        ...
                        <PanelColorSettings
                            ...
                            colorSettings={ [
                                ...
                                {
                                    label: __( 'Active Tab Background Color', 'aeopr' ),
                                    value: activeTabBgColor,
                                    onChange: ( value ) => setAttributes( { activeTabBgColor: value } ),
                                },
                                {
                                    label: __( 'Active Tab Text Color', 'aeopr' ),
                        ...
                        />
                        <PanelBody title={ __( 'Border Settings', 'aeopr' ) } initialOpen={ false }>
                            <SelectControl
                    ...
                    <div className={blockClass} style={ { border: 'none' } }>
                        <ul className="aeopr-tabs-panel">
                            {tabHeaders.map( ( item, index ) => (
                                <li key={ index }
                                    className={`aeopr-tab ${tabActive === index ? 'aeopr-tab-active' : ''}`}
                                    style={ {
                                ...
                                >
                                    ...
                                    >
                                        <RichText
                                            ...
                                            placeholder={ __( 'Title…', 'aeopr' ) }
                                        />
                                    </a>
                                    {tabHeaders.length > 1 && (
                                        <Tooltip text={ __( 'Remove tab', 'aeopr' ) }>
                            ...
                            <li className="aeopr-tab aeopr-add-tab"
                                style={ {
                                    borderRadius: borderRadius + 'px',
                                    borderWidth: borderWidth + 'px',
                                } }
                            >
                                <Tooltip text={ __( 'Add tab', 'aeopr' ) }>
                        ...
                        <div className="aeopr-tab-body-wrapper"
                             style={ {
                        ...
                        >
                            <InnerBlocks
                                template={ [ ['aeopr/tab'], ['aeopr/tab'], ['aeopr/tab']] }
                                templateLock={false}
                                allowedBlocks={ [ 'aeopr/tab' ] }
                            />
                        </div>
                    </div>
                    {!!pid &&
                    <style>
                        {activeTabBgColor && `#block-${clientId} li.aeopr-tab.aeopr-tab-active, #block-${clientId} li.aeopr-tab.ui-tabs-active {
                                background-color: ${activeTabBgColor} !important;
                            }`}
                        {activeTabTextColor && `#block-${clientId} li.aeopr-tab.aeopr-tab-active a, #block-${clientId} li.aeopr-tab.ui-tabs-active a {
    ...
    registerBlockType( 'aeopr/adv-tabs', {
        title: __( 'Advanced Tabs', 'aeopr' ),
        description: __( 'Create your own tabs never easy like this.', 'aeopr' ),
        icon: {
            src: tabsBlockIcon,
        },
        category: "aeopr",
                ...
                <div className={blockClass} data-tab-active={tabActiveFrontend}>
                    <ul className="aeopr-tabs-panel">
                        {tabHeaders.map( ( header, index ) => (
                            ...
                            >
                                <a 
                                	href={`#aeopr-tabs-tab${index}`}
									id={`aeopr-tabs-tab${index}`}
