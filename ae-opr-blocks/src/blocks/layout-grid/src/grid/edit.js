...
const ALLOWED_BLOCKS = [ 'aeopr/layout-grid-column' ];
...
class Edit extends Component {
	...
	renderDeviceSettings( columns, device, attributes ) {
		...
		for ( let column = 0; column < columns; column++ ) {
			...
			settings.push(
				<div className="aeopr-layout-grid-settings" key={ column }>
					<strong>
						{ __( 'Column', 'layout-grid' ) } { column + 1 }
					</strong>
					<div className="aeopr-layout-grid-settings__group">
						<TextControl
							type="number"
	...
	render() {
		...
		const classes = classnames(
			...
			extra,
			{
				'wp-block-aeopr-layout-tablet': previewMode === 'Tablet',
				'wp-block-aeopr-layout-desktop': previewMode === 'Desktop',
				'wp-block-aeopr-layout-mobile': previewMode === 'Mobile',
				'wp-block-aeopr-layout-resizable': this.canResizeBreakpoint(
					previewMode
				),
			...
			<>
				...
				<IsolatedEventContainer>
					...
					>
						...
						<InspectorControls>
							<PanelBody 
								title={ __( 'Layout', 'layout-grid' ) }
								
								>
								<div className="aeopr-layout-grid-columns block-editor-block-styles">
									{ getColumns().map( ( column ) => (
										<div
											key={ column.value }
							...
							<PanelBody
								title={__('Container Tag', 'layout-grid' )}
							...
							<PanelColorSettings
								title={ __( 'Background Color', 'layout-grid' ) }
	...
	if ( columns > previous ) {
		...
		return [
			...
			...times( columns - previous, () =>
				createBlock( 'aeopr/layout-grid-column' )
			),
		];
	}
