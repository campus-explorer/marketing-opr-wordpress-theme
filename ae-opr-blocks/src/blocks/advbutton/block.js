import {AdvColorControl} from "./components.js";
...
(function ( wpI18n, wpBlocks, wpElement, wpBlockEditor, wpComponents ) {
    ...
    class AdvButton extends Component {
        ...
        componentWillMount() {
           ...
           // const currentBlockConfig = aeoprDefaultConfig['aeopr-button'];
        ...
        componentDidMount() {
            ...
            if ( !attributes.id ) {
                setAttributes( { id: 'aeoprbtn-' + clientId } );
            }
        }

        render() {
            const listBorderStyles = [
                { label: __( 'None', 'aeopr' ), value: 'none' },
                { label: __( 'Solid', 'aeopr' ), value: 'solid' },
                { label: __( 'Dotted', 'aeopr' ), value: 'dotted' },
                { label: __( 'Dashed', 'aeopr' ), value: 'dashed' },
                { label: __( 'Double', 'aeopr' ), value: 'double' },
                { label: __( 'Groove', 'aeopr' ), value: 'groove' },
                { label: __( 'Ridge', 'aeopr' ), value: 'ridge' },
                { label: __( 'Inset', 'aeopr' ), value: 'inset' },
                { label: __( 'Outset', 'aeopr' ), value: 'outset' },
            ];
            const {
                attributes,
                setAttributes,
            ...
            return (
                isPreview ?
                    <img alt={__('Advanced Button', 'aeopr')} width='100%' src={previewImageData}/>
                    :
                    <Fragment>
                    <BlockControls>
                        ...
                        <Toolbar>
                            <IconButton
                                label={ __( 'Refresh this button when it conflict with other buttons styles', 'aeopr' ) }
                                icon="update"
                                className="components-toolbar__control"
                                onClick={ () => setAttributes( { id: 'aeoprbutton-' + blockID } ) }
                            />
                        </Toolbar>
                    </BlockControls>
                    ...
                    >
                        <RichText
                            tagName="span"
                            placeholder={ __( 'Add text…', 'aeopr' ) }
                            value={ text }
                            onChange={ ( value ) => setAttributes( { text: value } ) }
                            formattingControls={ [ 'bold', 'italic', 'strikethrough' ] }
                            isSelected={ isSelected }
                            className={ `wp-block-aeopr-button_link ${id}` }
                    ...
                    <InspectorControls>
                        <PanelBody title={ __( 'Button link', 'aeopr' ) }>
                            <BaseControl
                                label={ [
                                    __( 'Link URL', 'aeopr' ),
                            ...
                            <ToggleControl
                                label={ __( 'Open in new tab', 'aeopr' ) }
                                checked={ !!urlOpenNewTab }
                                onChange={ () => setAttributes( { urlOpenNewTab: !attributes.urlOpenNewTab } ) }
                            />
                        </PanelBody>
                        <PanelBody title={ __( 'Text/Color', 'aeopr' ) }>
                            <RangeControl
                                label={ __( 'Text size', 'aeopr' ) }
                                value={ textSize || '' }
                        ...
                        <PanelBody title={ __( 'Border', 'aeopr' ) } initialOpen={ false } >
                            {!isStyleSquared && (
                                <RangeControl
                                    label={ __( 'Border radius', 'aeopr' ) }
                                    value={ borderRadius || '' }
                        ...
                        <PanelBody title={ __( 'Margin', 'aeopr' ) } initialOpen={ false } >
                            <RangeControl
                                label={ __( 'Margin top', 'aeopr' ) }
                                value={ marginTop || '' }
                        ...
                        <PanelBody title={ __( 'Padding', 'aeopr' ) } initialOpen={ false } >
                            <RangeControl
                                label={ __( 'Padding top', 'aeopr' ) }
                                value={ paddingTop || '' }
                        ...
                        <PanelBody title={ __( 'Hover', 'aeopr' ) } initialOpen={ false } >
                            <PanelColorSettings
                                title={ __( 'Color Settings', 'aeopr' ) }
                                initialOpen={ false }
                                colorSettings={ hoverColorSettings }
                            />
                            <PanelBody title={ __( 'Shadow', 'aeopr' ) } initialOpen={ false }  >
                                <RangeControl
                                    label={ __('Opacity (%)', 'aeopr') }
                                    value={ hoverOpacity }
                                ...
                                />
                                <RangeControl
                                    label={ __('Transition speed (ms)', 'aeopr') }
                                ...
                                />
                                <RangeControl
                                    label={ __( 'Shadow H offset', 'aeopr' ) }
                                    value={ hoverShadowH || '' }
                                ...
                                />
                                <RangeControl
                                    label={ __( 'Shadow V offset', 'aeopr' ) }
                                    value={ hoverShadowV || '' }
    ...
    const blockAttrs = {
        ...
        text: {
            source: 'children',
            selector: 'a',
            default: __( 'PUSH THE BUTTON', 'aeopr' ),
        },
        bgColor: {
            type: 'string',
        },
    ...
    registerBlockType( 'aeopr/advancedbutton', {
        title: __( 'Advanced Button', 'aeopr' ),
        description: __( 'New button with more styles.', 'aeopr' ),
        icon: {
            src: buttonBlockIcon,
            foreground: typeof aeoprBlocks !== 'undefined' ? aeoprBlocks.color : undefined,
        },
        category: 'aeopr',
        keywords: [ __( 'button', 'aeopr' ), __( 'link', 'aeopr' ) ],
            ...
            from: [
                {
                    ...
                    transform: ( attributes ) => {
                        return createBlock( 'aeopr/button', {
                            ...attributes,
        ...
        styles: [
            { name: 'default', label: __( 'Default', 'aeopr' ), isDefault: true },
            { name: 'outlined', label: __( 'Outlined', 'aeopr' ) },
            { name: 'squared', label: __( 'Squared', 'aeopr' ) },
            { name: 'squared-outline', label: __( 'Squared Outline', 'aeopr' ) },
        ],
        edit: AdvButton,
