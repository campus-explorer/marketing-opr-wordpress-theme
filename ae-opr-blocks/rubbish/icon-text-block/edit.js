...
import ArcherOPRIcon from "../../../dist/blocks/controls/ArcherOPRIcons.json"//icons for fonticonpicker
...
let svg_icons = Object.keys( ArcherOPRIcon )
...
class AEOPRIconListChild extends Component {
	...
	render() {
		...
		const renderHtml = () => {
			...
			if ( image_icon == "icon" ) {
				if ( icon ) {
					image_icon_html = <span className="aeopr-icon-block__source-icon">{ renderSVG(icon) }</span>
				}
			} else {
				if ( image && image.url ) {
					image_icon_html = <img className="aeopr-icon-block__source-image" src={image.url} />
				}
			}
			...
			return (
				...
				>
					...
					<div className="aeopr-icon-block__content-wrap">
						<span className="aeopr-icon-block__source-wrap">{image_icon_html}</span>
						{ ! hideLabel && "" != label &&
