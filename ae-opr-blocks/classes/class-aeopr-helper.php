...
if ( ! class_exists( 'AEOPR_Helper' ) ) {

	/**
	 * Class AEOPR_Helper.
	 */
	final class AEOPR_Helper {
		...
		public function __construct() {
			...
			require AEOPR_DIR . 'classes/class-aeopr-config.php';
			require AEOPR_DIR . 'classes/class-aeopr-block-helper.php';
			require AEOPR_DIR . 'classes/class-aeopr-block-js.php';

			self::$block_list      = AEOPR_Config::get_block_attributes();
		...
		public function block_assets() {
			...
			if ( 'enabled' === self::$file_generation ) {
				...
				if ( isset( $file_handler['css_url'] ) ) {
					wp_enqueue_style( 'uag-style', $file_handler['css_url'], array(), AEOPR_VER, 'all' );
				} else {
					self::$fallback_css = true;
				}
				if ( isset( $file_handler['js_url'] ) ) {
					wp_enqueue_script( 'uag-script', $file_handler['js_url'], array(), AEOPR_VER, true );
				} else {
		...
		public function print_script() {
			...
			ob_start();
			?>
			<script type="text/javascript" id="aeopr-script-frontend"><?php echo self::$script; //phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped ?></script>
			<?php
			ob_end_flush();
		}
		...
		public function print_stylesheet() {
			...
			ob_start();
			?>
			<style id="aeopr-style-frontend"><?php echo self::$stylesheet; //phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped ?></style>
			<?php
			ob_end_flush();
		}
		...
		public function frontend_gfonts() {
			...
			$show_google_fonts = apply_filters( 'aeopr_blocks_show_google_fonts', true );
		...
		public function get_block_css_and_js( $block ) {
			...
			if ( strpos( $name, 'aeopr/' ) !== false ) {
				self::$uag_flag = true;
			}

			switch ( $name ) {
				case 'aeopr/how-to':
					$css += AEOPR_Block_Helper::get_how_to_css( $blockattr, $block_id );
					AEOPR_Block_JS::blocks_how_to_gfont( $blockattr );
					break;

				case 'aeopr/section':
					$css += AEOPR_Block_Helper::get_section_css( $blockattr, $block_id );
					break;

				case 'aeopr/advanced-heading':
					$css += AEOPR_Block_Helper::get_adv_heading_css( $blockattr, $block_id );
					AEOPR_Block_JS::blocks_advanced_heading_gfont( $blockattr );
					break;

				case 'aeopr/info-box':
					$css += AEOPR_Block_Helper::get_info_box_css( $blockattr, $block_id );
					AEOPR_Block_JS::blocks_info_box_gfont( $blockattr );
					break;

				case 'aeopr/buttons':
					$css += AEOPR_Block_Helper::get_buttons_css( $blockattr, $block_id );
					AEOPR_Block_JS::blocks_buttons_gfont( $blockattr );
					break;

				case 'aeopr/buttons-child':
					$css += AEOPR_Block_Helper::get_buttons_child_css( $blockattr, $block_id );
					break;

				case 'aeopr/blockquote':
					$css += AEOPR_Block_Helper::get_blockquote_css( $blockattr, $block_id );
					AEOPR_Block_JS::blocks_blockquote_gfont( $blockattr );
					$js .= AEOPR_Block_JS::get_blockquote_js( $blockattr, $block_id );
					break;

				case 'aeopr/testimonial':
					$css += AEOPR_Block_Helper::get_testimonial_css( $blockattr, $block_id );
					AEOPR_Block_JS::blocks_testimonial_gfont( $blockattr );
					$js .= AEOPR_Block_JS::get_testimonial_js( $blockattr, $block_id );
					break;

				case 'aeopr/team':
					$css += AEOPR_Block_Helper::get_team_css( $blockattr, $block_id );
					AEOPR_Block_JS::blocks_team_gfont( $blockattr );
					break;

				case 'aeopr/social-share':
					$css += AEOPR_Block_Helper::get_social_share_css( $blockattr, $block_id );
					$js  .= AEOPR_Block_JS::get_social_share_js( $blockattr, $block_id );
					break;

				case 'aeopr/social-share-child':
					$css += AEOPR_Block_Helper::get_social_share_child_css( $blockattr, $block_id );
					break;

				case 'aeopr/content-timeline':
					$css += AEOPR_Block_Helper::get_content_timeline_css( $blockattr, $block_id );
					AEOPR_Block_JS::blocks_content_timeline_gfont( $blockattr );
					break;

				case 'aeopr/restaurant-menu':
					$css += AEOPR_Block_Helper::get_restaurant_menu_css( $blockattr, $block_id );
					AEOPR_Block_JS::blocks_restaurant_menu_gfont( $blockattr );
					break;

				case 'aeopr/call-to-action':
					$css += AEOPR_Block_Helper::get_call_to_action_css( $blockattr, $block_id );
					AEOPR_Block_JS::blocks_call_to_action_gfont( $blockattr );
					break;

				case 'aeopr/post-timeline':
					$css += AEOPR_Block_Helper::get_post_timeline_css( $blockattr, $block_id );
					AEOPR_Block_JS::blocks_post_timeline_gfont( $blockattr );
					break;

				case 'aeopr/icon-list':
					$css += AEOPR_Block_Helper::get_icon_list_css( $blockattr, $block_id );
				...
				case 'aeopr/icon-list-child':
					$css += AEOPR_Block_Helper::get_icon_list_child_css( $blockattr, $block_id );
					break;

				case 'aeopr/post-grid':
					$css += AEOPR_Block_Helper::get_post_grid_css( $blockattr, $block_id );
					AEOPR_Block_JS::blocks_post_gfont( $blockattr );
					break;

				case 'aeopr/post-carousel':
					$css += AEOPR_Block_Helper::get_post_carousel_css( $blockattr, $block_id );
					AEOPR_Block_JS::blocks_post_gfont( $blockattr );
					break;

				case 'aeopr/post-masonry':
					$css += AEOPR_Block_Helper::get_post_masonry_css( $blockattr, $block_id );
					AEOPR_Block_JS::blocks_post_gfont( $blockattr );
					break;

				case 'aeopr/columns':
					$css += AEOPR_Block_Helper::get_columns_css( $blockattr, $block_id );
					break;

				case 'aeopr/column':
					$css += AEOPR_Block_Helper::get_column_css( $blockattr, $block_id );
					break;

				case 'aeopr/cf7-styler':
					$css += AEOPR_Block_Helper::get_cf7_styler_css( $blockattr, $block_id );
					AEOPR_Block_JS::blocks_cf7_styler_gfont( $blockattr );
					break;

				case 'aeopr/marketing-button':
					$css += AEOPR_Block_Helper::get_marketing_btn_css( $blockattr, $block_id );
					AEOPR_Block_JS::blocks_marketing_btn_gfont( $blockattr );
					break;

				case 'aeopr/gf-styler':
					$css += AEOPR_Block_Helper::get_gf_styler_css( $blockattr, $block_id );
					AEOPR_Block_JS::blocks_gf_styler_gfont( $blockattr );
					break;

				case 'aeopr/table-of-contents':
					$css += AEOPR_Block_Helper::get_table_of_contents_css( $blockattr, $block_id );
					AEOPR_Block_JS::blocks_table_of_contents_gfont( $blockattr );
					$js .= AEOPR_Block_JS::get_table_of_contents_js( $blockattr, $block_id );
					break;

				case 'aeopr/faq':
					$css += AEOPR_Block_Helper::get_faq_css( $blockattr, $block_id );
					AEOPR_Block_JS::blocks_faq_gfont( $blockattr );
					break;
		...
		public function generate_assets() {
			...
			if ( is_single() || is_page() || is_404() ) {
				 ...
				 */
				$this_post = apply_filters( 'aeopr_post_for_stylesheet', $this_post );
		...
		public function get_assets( $blocks ) {
			...
			if ( ! empty( $tablet ) ) {
				$tab_styling_css .= '@media only screen and (max-width: ' . AEOPR_TABLET_BREAKPOINT . 'px) {';
				$tab_styling_css .= $tablet;
				$tab_styling_css .= '}';
			}

			if ( ! empty( $mobile ) ) {
				$mob_styling_css .= '@media only screen and (max-width: ' . AEOPR_MOBILE_BREAKPOINT . 'px) {';
		...
		public static function backend_load_font_awesome_icons() {
			...
			$json_file = AEOPR_DIR . 'dist/blocks/aeopr-controls/AEOPRIcon.json';
		...
		public static function get_query( $attributes, $block_type ) {
			...
			$query_args = apply_filters( "aeopr_post_query_args_{$block_type}", $query_args, $attributes );
		...
		public static function get_image_sizes() {
			...
			$image_sizes = apply_filters( 'aeopr_post_featured_image_sizes', $image_sizes );
		...
		public static function get_post_types() {
			...
			return apply_filters( 'aeopr_loop_post_types', $options );
		}
		...
		public static function get_related_taxonomy() {
			...
			return apply_filters( 'aeopr_post_loop_taxonomies', $return_array );
		}
		...
		public static function allow_file_generation() {
			return get_option( '_aeopr_allow_file_generation', 'disabled' );
		}
	...
	/**
	 *  Prepare if class 'AEOPR_Helper' exist.
	 *  Kicking this off by calling 'get_instance()' method
	 */
	AEOPR_Helper::get_instance();
}
