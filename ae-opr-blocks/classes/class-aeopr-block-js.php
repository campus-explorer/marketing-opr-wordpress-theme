...
if ( ! class_exists( 'AEOPR_Block_JS' ) ) {

	/**
	 * Class AEOPR_Block_JS.
	 */
	class AEOPR_Block_JS {
		...
		public static function get_testimonial_js( $attr, $id ) {
			...
			$slick_options = apply_filters(
				'aeopr_testimonials_slick_options',
				array(
			...
			$base_selector = ( isset( $attr['classMigrate'] ) && $attr['classMigrate'] ) ? '.aeopr-block-' : '#aeopr-testimonial-';
			$selector      = $base_selector . $id;
			$js            = null;//'$( document ).ready( function() { if( $( "' . $selector . '" ).length > 0 ){ $( "' . $selector . '" ).find( ".is-carousel" ).slick( ' . $settings . ' ); } } );';
		...
		public static function get_blockquote_js( $attr, $id ) {
			...
			$base_selector = ( isset( $attr['classMigrate'] ) && $attr['classMigrate'] ) ? '.aeopr-block-' : '#aeopr-blockquote-';
			$selector      = $base_selector . $id;

			$js = 'jQuery( "' . $selector . '" ).find( ".aeopr-blockquote__tweet-button" ).click(function(){ var content = jQuery("' . $selector . '").find(".aeopr-blockquote__content").text(); var request_url = "https://twitter.com/share?url="+ encodeURIComponent("' . $url . '")+"&text="+content+"&via="+("' . $via . '"); window.open( request_url ); });';
		...
		public static function get_social_share_js( $attr, $id ) {
			...
			$base_selector = ( isset( $attr['classMigrate'] ) && $attr['classMigrate'] ) ? '.aeopr-block-' : '#aeopr-social-share-';
			$selector      = $base_selector . $id;
			ob_start();
			?>
			var ssLinks = document.querySelectorAll( '<?php echo esc_attr( $selector ); ?>' );
			for ( var j = 0; j < ssLinks.length; j++ ) {
				var ssLink = ssLinks[j].querySelectorAll( ".aeopr-ss__link" );
		...
		public static function get_table_of_contents_js( $attr, $id ) {
			...
			$attr          = array_merge( $defaults, (array) $attr );
			$base_selector = ( isset( $attr['classMigrate'] ) && $attr['classMigrate'] ) ? '.aeopr-block-' : '#aeopr-toc-';
			$selector      = $base_selector . $id;

			ob_start();
			?>
			jQuery( document ).ready(function() {
				AEOPRTableOfContents._run( <?php echo wp_json_encode( $attr ); ?>, '<?php echo esc_attr( $selector ); ?>' );
			});
			<?php
			return ob_get_clean();
		...
		public static function blocks_advanced_heading_gfont( $attr ) {
			...
			AEOPR_Helper::blocks_google_font( $head_load_google_font, $head_font_family, $head_font_weight, $head_font_subset );
			AEOPR_Helper::blocks_google_font( $subhead_load_google_font, $subhead_font_family, $subhead_font_weight, $subhead_font_subset );
		}
		...
		public static function blocks_how_to_gfont( $attr ) {
			...
			AEOPR_Helper::blocks_google_font( $head_load_google_font, $head_font_family, $head_font_weight, $head_font_subset );
			AEOPR_Helper::blocks_google_font( $subhead_load_google_font, $subhead_font_family, $subhead_font_weight, $subhead_font_subset );
			AEOPR_Helper::blocks_google_font( $price_load_google_font, $price_font_family, $price_font_weight, $price_font_subset );
		}
		...
		public static function blocks_cf7_styler_gfont( $attr ) {
			...
			AEOPR_Helper::blocks_google_font( $msg_font_load_google_font, $msg_font_family, $msg_font_weight, $msg_font_subset );
			AEOPR_Helper::blocks_google_font( $validation_msg_load_google_font, $validation_msg_font_family, $validation_msg_font_weight, $validation_msg_font_subset );

			AEOPR_Helper::blocks_google_font( $radio_check_load_google_font, $radio_check_font_family, $radio_check_font_weight, $radio_check_font_subset );
			AEOPR_Helper::blocks_google_font( $button_load_google_font, $button_font_family, $button_font_weight, $button_font_subset );

			AEOPR_Helper::blocks_google_font( $label_load_google_font, $label_font_family, $label_font_weight, $label_font_subset );
			AEOPR_Helper::blocks_google_font( $input_load_google_font, $input_font_family, $input_font_weight, $input_font_subset );
		}
		...
		public static function blocks_marketing_btn_gfont( $attr ) {
			...
			AEOPR_Helper::blocks_google_font( $title_load_google_font, $title_font_family, $title_font_weight, $title_font_subset );
			AEOPR_Helper::blocks_google_font( $prefix_load_google_font, $prefix_font_family, $prefix_font_weight, $prefix_font_subset );
		}
		...
		public static function blocks_table_of_contents_gfont( $attr ) {
			...
			AEOPR_Helper::blocks_google_font( $load_google_font, $font_family, $font_weight, $font_subset );
			AEOPR_Helper::blocks_google_font( $heading_load_google_font, $heading_font_family, $heading_font_weight, $heading_font_subset );
		}
		...
		public static function blocks_blockquote_gfont( $attr ) {
			...
			AEOPR_Helper::blocks_google_font( $desc_load_google_font, $desc_font_family, $desc_font_weight, $desc_font_subset );
			AEOPR_Helper::blocks_google_font( $author_load_google_font, $author_font_family, $author_font_weight, $author_font_subset );
			AEOPR_Helper::blocks_google_font( $tweet_btn_load_google_font, $tweet_btn_font_family, $tweet_btn_font_weight, $tweet_btn_font_subset );
		}
		...
		public static function blocks_testimonial_gfont( $attr ) {
			...
			AEOPR_Helper::blocks_google_font( $desc_load_google_fonts, $desc_font_family, $desc_font_weight, $desc_font_subset );
			AEOPR_Helper::blocks_google_font( $name_load_google_fonts, $name_font_family, $name_font_weight, $name_font_subset );
			AEOPR_Helper::blocks_google_font( $company_load_google_fonts, $company_font_family, $company_font_weight, $company_font_subset );
		}
		...
		public static function blocks_restaurant_menu_gfont( $attr ) {
			...
			AEOPR_Helper::blocks_google_font( $title_load_google_fonts, $title_font_family, $title_font_weight, $title_font_subset );
			AEOPR_Helper::blocks_google_font( $price_load_google_fonts, $price_font_family, $price_font_weight, $price_font_subset );
			AEOPR_Helper::blocks_google_font( $desc_load_google_fonts, $desc_font_family, $desc_font_weight, $desc_font_subset );
		}
		...
		public static function blocks_content_timeline_gfont( $attr ) {
			...
			AEOPR_Helper::blocks_google_font( $head_load_google_fonts, $head_font_family, $head_font_weight, $head_font_subset );
			AEOPR_Helper::blocks_google_font( $subheadload_google_fonts, $subheadfont_family, $subheadfont_weight, $subheadfont_subset );
			AEOPR_Helper::blocks_google_font( $date_load_google_fonts, $date_font_family, $date_font_weight, $date_font_subset );
		}
		...
		public static function blocks_post_timeline_gfont( $attr ) {
			...
			AEOPR_Helper::blocks_google_font( $author_load_google_fonts, $author_font_family, $author_font_weight, $author_font_subset );
			AEOPR_Helper::blocks_google_font( $cta_load_google_fonts, $cta_font_family, $cta_font_weight, $cta_font_subset );
		}
		...
		public static function blocks_post_gfont( $attr ) {
			...
			AEOPR_Helper::blocks_google_font( $meta_load_google_font, $meta_font_family, $meta_font_weight, $meta_font_subset );

			AEOPR_Helper::blocks_google_font( $excerpt_load_google_font, $excerpt_font_family, $excerpt_font_weight, $excerpt_font_subset );

			AEOPR_Helper::blocks_google_font( $cta_load_google_font, $cta_font_family, $cta_font_weight, $cta_font_subset );
		}
		...
		public static function blocks_faq_gfont( $attr ) {
			...
			AEOPR_Helper::blocks_google_font( $question_load_google_font, $question_font_family, $question_font_weight, $question_font_subset );
			AEOPR_Helper::blocks_google_font( $answer_load_google_font, $answer_font_family, $answer_font_weight, $answer_font_subset );
