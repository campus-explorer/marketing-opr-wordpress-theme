 ...
 */
	 ...
	 * Load Post Grid PHP
	 */
	require_once plugin_dir_path( __FILE__ ) . 'src/blocks/programs-grid/index.php';
...
function aeopr_blocks_enqueue_editor_assets(){
    wp_enqueue_script(
     ...
     wp_localize_script(
		...
		'aeopr_settings',
		array(
			'start_date'		=> get_option('opr_brand_settings_programs_start_date'),
			'apply_date'		=> get_option('opr_brand_settings_programs_apply_date'),
			'client_icon_library' => get_option('opr_brand_settings_icon_library'),
			'google_api_key'	=> 'AIzaSyBZHDQhDtWTvbk_yW4-EqV9AaIMGVxAC5o',
			'redirect'			=>	'https://xapi.view.do/v1/experience/link/vb-edu-rfi-peru/org?useExisting=true&utm_source=online.peru.edu&utm_medium=referral&campaignKey=ps&', /// Post form redirect url
			'clientCTPA'=>	get_option('opr_brand_settings_global_ctpa')
    ...
    $prefix_heading_font = 'Arial, sans-serif';//get_theme_mod( 'heading_font', 'Lora' );

	$prefix_body_font = 'Arial,sans-serif';//get_theme_mod( 'body_font', 'Roboto' );
