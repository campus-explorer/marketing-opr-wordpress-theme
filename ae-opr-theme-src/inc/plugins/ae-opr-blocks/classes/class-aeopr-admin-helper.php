...
if ( ! class_exists( 'AEOPR_Admin_Helper' ) ) {

	/**
	 * Class AEOPR_Admin_Helper.
	 */
	final class AEOPR_Admin_Helper {
		...
		public static function get_block_options() {
			...
			$blocks       = AEOPR_Helper::$block_list;
			$saved_blocks = self::get_admin_settings_option( '_aeopr_blocks' );
			if ( is_array( $blocks ) ) {
			...
			AEOPR_Helper::$block_list = $blocks;

			return apply_filters( 'uagb_enabled_blocks', AEOPR_Helper::$block_list );
		}
		...
		public static function create_specific_stylesheet() {
			...
			foreach ( AEOPR_Config::$block_attributes as $key => $block ) {
			...
			$combined_path = plugin_dir_path( AEOPR_FILE ) . 'dist/blocks.style.css';
			wp_delete_file( $combined_path );

			$style = '';

			$wp_filesystem = AEOPR_Helper::get_instance()->get_filesystem();

			foreach ( $combined as $key => $c_block ) {
				$style .= $wp_filesystem->get_contents( plugin_dir_path( AEOPR_FILE ) . 'assets/css/blocks/' . $c_block . '.css' );
	...
	/**
	 *  Prepare if class 'AEOPR_Admin_Helper' exist.
	 *  Kicking this off by calling 'get_instance()' method
	 */
	AEOPR_Admin_Helper::get_instance();
}
