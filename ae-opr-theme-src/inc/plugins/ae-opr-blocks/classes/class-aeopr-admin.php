...
if ( ! class_exists( 'AEOPR_Admin' ) ) {

	/**
	 * Class AEOPR_Admin.
	 */
	final class AEOPR_Admin {
		...
		public static function init() {
			...
			// Add AEOPR menu option to admin.
			add_action( 'network_admin_menu', __CLASS__ . '::menu' );

			add_action( 'admin_menu', __CLASS__ . '::menu' );

			add_action( 'aeopr_render_admin_content', __CLASS__ . '::render_content' );
			...
			add_action( 'wp_ajax_aeopr_file_generation', __CLASS__ . '::file_generation' );

			// Enqueue admin scripts.
			if ( isset( $_GET['page'] ) && AEOPR_SLUG === $_GET['page'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		public static function activation_redirect() {
			$do_redirect = apply_filters( 'aeopr_enable_redirect_activation', get_option( '__aeopr_do_redirect' ) );
			if ( $do_redirect ) {
				update_option( '__aeopr_do_redirect', false );
				if ( ! is_multisite() ) {
					wp_safe_redirect( esc_url( admin_url( 'options-general.php?page=' . AEOPR_SLUG ) ) );
					exit();
				}
			}
		}
		...
		public static function register_notices() {
			...
			$image_path = AEOPR_URL . 'admin/assets/images/aeopr_notice.svg';

			Astra_Notices::add_notice(
				array(
					'id'                         => 'aeopr-admin-rating',
					'type'                       => '',
					'message'                    => sprintf(
						'<div class="notice-image">
							...
							<div class="notice-content">
								...
								%3$s<br />
								<div class="aeopr-review-notice-container">
									<a href="%4$s" class="astra-notice-close aeopr-review-notice button-primary" target="_blank">
									%5$s
									</a>
								<span class="dashicons dashicons-calendar"></span>
									<a href="#" data-repeat-notice-after="%6$s" class="astra-notice-close aeopr-review-notice">
									%7$s
									</a>
								<span class="dashicons dashicons-smiley"></span>
									<a href="#" class="astra-notice-close aeopr-review-notice">
									%8$s
									</a>
								</div>
							</div>',
					...
					'show_if'                    => AEOPR_Admin_Helper::show_rating_notice(),
				)
			);
		...
		public static function render() {
			...
			$aeopr_icon                 = apply_filters( 'aeopr_header_top_icon', true );
			$aeopr_visit_site_url       = apply_filters( 'aeopr_site_url', 'https://www.ultimategutenberg.com/?utm_source=uag-dashboard&utm_medium=link&utm_campaign=uag-dashboard' );
			$aeopr_header_wrapper_class = apply_filters( 'aeopr_header_wrapper_class', array( $action ) );

			include_once AEOPR_DIR . 'admin/aeopr-admin.php';
		}
		...
		public static function render_content() {
			...
			$base_path = realpath( AEOPR_DIR . '/admin' );
			$path      = realpath( $base_path . '/aeopr-' . $action . '.php' );
		...
		public static function notice_styles_scripts() {
			// Styles.
			wp_enqueue_style( 'aeopr-notice-settings', AEOPR_URL . 'admin/assets/admin-notice.css', array(), AEOPR_VER );
		}
		...
		public static function styles_scripts() {
			...
			// Styles.
			wp_enqueue_style( 'aeopr-admin-settings', AEOPR_URL . 'admin/assets/admin-menu-settings.css', array(), AEOPR_VER, 'all' );
			// Script.
			wp_enqueue_script( 'aeopr-admin-settings', AEOPR_URL . 'admin/assets/admin-menu-settings.js', array( 'jquery', 'wp-util', 'updates' ), AEOPR_VER, true );

			$localize = array(
				'ajax_url'        => admin_url( 'admin-ajax.php' ),
				'ajax_nonce'      => wp_create_nonce( 'aeopr-block-nonce' ),
			...
			wp_localize_script( 'aeopr-admin-settings', 'aeopr', apply_filters( 'aeopr_js_localize', $localize ) );
		}
		...
		public static function save_settings() {
			...
			do_action( 'aeopr_admin_settings_save' );
		}
		...
		public static function initialize_ajax() {
			...
			// Ajax requests.
			add_action( 'wp_ajax_aeopr_activate_widget', __CLASS__ . '::activate_widget' );
			add_action( 'wp_ajax_aeopr_deactivate_widget', __CLASS__ . '::deactivate_widget' );

			add_action( 'wp_ajax_aeopr_bulk_activate_widgets', __CLASS__ . '::bulk_activate_widgets' );
			add_action( 'wp_ajax_aeopr_bulk_deactivate_widgets', __CLASS__ . '::bulk_deactivate_widgets' );

			add_action( 'wp_ajax_aeopr_allow_beta_updates', __CLASS__ . '::allow_beta_updates' );
		}
		...
		public static function activate_widget() {
			...
			check_ajax_referer( 'aeopr-block-nonce', 'nonce' );

			$block_id            = sanitize_text_field( $_POST['block_id'] );
			$blocks              = AEOPR_Admin_Helper::get_admin_settings_option( '_aeopr_blocks', array() );
			$blocks[ $block_id ] = $block_id;
			$blocks              = array_map( 'esc_attr', $blocks );

			// Update blocks.
			AEOPR_Admin_Helper::update_admin_settings_option( '_aeopr_blocks', $blocks );
		...
		public static function bulk_activate_widgets() {
			...
			// Get all widgets.
			$all_blocks = AEOPR_Helper::$block_list;
			$new_blocks = array();
			...
			AEOPR_Admin_Helper::update_admin_settings_option( '_aeopr_blocks', $new_blocks );
			AEOPR_Admin_Helper::create_specific_stylesheet();
		...
		public static function bulk_deactivate_widgets() {
			...
			// Get all extensions.
			$old_blocks = AEOPR_Helper::$block_list;
			$new_blocks = array();
		...
		public static function allow_beta_updates() {
			...
			// Update new_extensions.
			AEOPR_Admin_Helper::update_admin_settings_option( '_aeopr_beta', $beta_update );
		...
		public static function file_generation() {
			...
			wp_send_json_success(
				array(
					'success' => true,
					'message' => update_option( '_aeopr_allow_file_generation', $_POST['value'] ),
				)
			);
		}
